---
title: Gameplay
description: Mods de gameplay du modpack Wildborne - Combat, cuisine, compétences et plus
---

# :crossed_swords: Gameplay

Cette section couvre les **24 mods** qui modifient les mécaniques de jeu : combat, cuisine, compétences, artisanat, survie et génération du monde.

---

## Catégories

<div class="category-grid" markdown>

<div class="category-card" markdown>

### :keyboard: Raccourcis Clavier
**Guide essentiel**

Toutes les touches et commandes importantes des mods.

[:octicons-arrow-right-24: Voir les raccourcis](raccourcis.md)

</div>

<div class="category-card" markdown>

### :cook: Cuisine
**7 mods**

Système culinaire complet avec nouveaux aliments, ustensiles et recettes.

[:octicons-arrow-right-24: Voir la cuisine](cuisine.md)

</div>

<div class="category-card" markdown>

### :crossed_swords: Combat
**6 mods**

Refonte complète du combat avec nouvelles armes et mécaniques.

[:octicons-arrow-right-24: Voir le combat](combat.md)

</div>

<div class="category-card" markdown>

### :chart_with_upwards_trend: Compétences
**4 mods**

Système de progression par compétences et classes.

[:octicons-arrow-right-24: Voir les compétences](competences.md)

</div>

<div class="category-card" markdown>

### :hammer: Artisanat
**6 mods**

Outils de construction et artisanat avancés.

[:octicons-arrow-right-24: Voir l'artisanat](artisanat.md)

</div>

<div class="category-card" markdown>

### :droplet: Survie
**2 mods**

Mécaniques de survie réalistes.

[:octicons-arrow-right-24: Voir la survie](survie.md)

</div>

<div class="category-card" markdown>

### :world_map: Monde
**3 mods**

Génération du monde et structures.

[:octicons-arrow-right-24: Voir le monde](monde.md)

</div>

</div>

---

## Résumé des Mods

### Cuisine & Alimentation (8 mods)

| Mod | Version | Description |
|-----|---------|-------------|
| aculinaryartillery | 2.0.0-dev.11 | Arsenal culinaire avancé |
| expandedfoods | 2.0.0-dev.6 | Aliments étendus |
| diversediets | 1.0.3 | Régimes alimentaires variés |
| metalpots | 1.6.3 | Casseroles en métal |
| stonebakeoven | 1.2.0 | Four en pierre |
| betterfirepit | 1.2.0 | Foyer amélioré |
| pipeleaf | 2.4.1 | Tabac et pipes |

### Combat & Armes (6 mods)

| Mod | Version | Description |
|-----|---------|-------------|
| combatoverhaul | 0.10.19 | Refonte du système de combat |
| armory | 1.6.2 | Arsenal d'armes |
| maltiezcrossbows | 1.6.8 | Arbalètes |
| maltiezfirearms | 1.5.4 | Armes à feu |
| cooperativecombatrework | 1.0.0 | Combat coopératif |
| coopsynergy | 0.1.0 | Synergies coop |

### Compétences & Progression (4 mods)

| Mod | Version | Description |
|-----|---------|-------------|
| xskills | 0.9.0-pre.2 | Système de compétences |
| xskillsgilded | 1.3.5 | Extension dorée de XSkills |
| traitacquirer | 0.9.6 | Acquisition de traits |
| theworkingclasses | 1.0.4 | Classes de métiers |

### Artisanat & Construction (6 mods)

| Mod | Version | Description |
|-----|---------|-------------|
| chiseltools | 1.15.6 | Outils de ciselage avancés |
| cbr | 2.0.1 | Blocs ciselés conservent la chaleur |
| stonequarry | 3.5.1 | Carrière de pierre |
| millwright | 1.3.1 | Moulins et mécanismes |
| smithingplus | 1.8.3 | Forge améliorée |
| toolsmith | 1.2.14 | Fabrication d'outils |

### Survie & Réalisme (2 mods)

| Mod | Version | Description |
|-----|---------|-------------|
| hydrateordiedrate | 2.3.4 | Système d'hydratation |
| primitivesurvival | 4.0.2 | Survie primitive |

### Monde & Génération (3 mods)

| Mod | Version | Description |
|-----|---------|-------------|
| betterruins | 0.5.4 | Ruines améliorées |
| rivers | 4.7.3 | Système de rivières |
| bettertraders | 0.1.0 | Marchands améliorés |

---

## Synergies Importantes

!!! info "Combat + Compétences"
    Les mods XSkills s'intègrent avec Combat Overhaul pour des bonus de dégâts basés sur vos compétences.

!!! info "Cuisine + Survie"
    Hydrate or Diedrate ajoute une jauge de soif que vous pouvez gérer avec les boissons de Expanded Foods.
