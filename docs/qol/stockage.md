---
title: Stockage & Organisation
description: Mods de stockage du modpack Wildborne
---

# :package: Stockage & Organisation

**8 mods** pour mieux organiser vos ressources et votre base.

---

## Liste des Mods

| Mod | Version | Description |
|-----|---------|-------------|
| carryon | 1.12.1 | Porter des blocs et conteneurs |
| foodshelves | 2.3.3 | Étagères dédiées à la nourriture |
| shelfobsessed | 1.9.1 | Variété d'étagères décoratives |
| sortablestorage | 2.4.4 | Tri automatique des coffres |
| morepiles | 2.2.0 | Empiler plus de ressources au sol |
| purposefulstorage | 1.5.1 | Conteneurs spécialisés |
| tankardsandgoblets | 1.3.4 | Rangement de boissons |
| storageoptions | 2.0.0 | Râteliers d'armes et étagères simples |

---

## Carry On

![Carry On](https://moddbcdn.vintagestory.at/CarryOnLogo_aef8a4cbdf80f8b3851b5dc3b23e1a28.png){ align=right width=100 }

Ramassez et transportez des blocs entiers, **conteneurs pleins inclus**.

### Contrôles
| Action | Touches |
|--------|---------|
| Porter en main | Shift + Clic droit (mains vides) |
| Porter sur le dos | Paniers et petits conteneurs |
| Lâcher rapidement | CTRL+ALT+K |

### Objets transportables
- Coffres, paniers, caisses avec leur contenu
- Enclumes, établis, machines
- Tonneaux pleins de liquides
- Vases et conteneurs divers

!!! warning "Pénalités"
    Porter ralentit vos mouvements et empêche de courir. Les dégâts font tomber l'objet !

??? info "Mods liés"
    - :material-arrow-right: [A Culinary Artillery](../gameplay/cuisine.md#a-culinary-artillery) : Transport des ustensiles de cuisine
    - :material-arrow-right: [Animal Cages](../faune/interactions/index.md#animal-cages) : Transport des cages pleines
    - :material-arrow-right: [Chisel Tools](../gameplay/artisanat.md#chisel-tools) : Transport des blocs ciselés

---

## Sortable Storage

![Sortable Storage](https://moddbcdn.vintagestory.at/logo3_64d95975ac75950279208cd595e7f1e4.png){ align=right width=100 }

Transformez vos conteneurs vanilla en versions **triables automatiquement**.

### Utilisation
1. Placez un coffre/panier vanilla dans la grille de craft
2. Récupérez la version "sortable"
3. Utilisez le **bouton de tri** (coin supérieur droit)

### Fonctionnalités
- Tri par **nom d'affichage** alphabétique
- **Fusion automatique** des piles similaires
- Compatible avec Carry On et autres mods de stockage

??? info "Mods liés"
    - :material-arrow-right: [Carry On](#carry-on) : Déplacement des coffres triables
    - :material-arrow-right: [Food Shelves](#food-shelves) : Étagères de nourriture triables

---

## Food Shelves

![Food Shelves](https://moddbcdn.vintagestory.at/thumbnailsmallnew_b2ff8d7ef92ea2060f7acb49a35f4ab2.png){ align=right width=100 }

Étagères spécialement conçues pour la nourriture avec **préservation améliorée**.

- **Affichage visuel** des aliments stockés
- **Conservation prolongée** par rapport aux coffres
- **Accès rapide** sans ouvrir d'interface
- Design rustique authentique

??? info "Mods liés"
    - :material-arrow-right: [Expanded Foods](../gameplay/cuisine.md#expanded-foods) : Affichage des nouveaux aliments
    - :material-arrow-right: [A Culinary Artillery](../gameplay/cuisine.md#a-culinary-artillery) : Stockage des ingrédients de cuisine

---

## More Piles

![More Piles](https://moddbcdn.vintagestory.at/MorePilesSplash_ba37df0fa7ef5cbedfa91538d8440e40.png){ align=right width=100 }

Stockez plus de ressources directement au sol de manière esthétique.

- **Plus de matériaux** empilables (lingots, planches, etc.)
- **Piles plus grandes** que vanilla
- **Aspect décoratif** pour vos ateliers
- Organisation visuelle de vos ressources

??? info "Mods liés"
    - :material-arrow-right: [Stone Quarry](../gameplay/artisanat.md#stone-quarry) : Stockage des matériaux de construction
    - :material-arrow-right: [Smithing Plus](../gameplay/artisanat.md#smithing-plus) : Piles de fragments métalliques
    - :material-arrow-right: [Material Needs](../gameplay/monde.md#material-needs) : Plus de variantes empilables

---

## Storage Options

Conteneurs supplémentaires pour organiser votre équipement.

- **Râtelier d'armes** avec 4 emplacements
- **Étagère simple** avec 1 emplacement
- Affichage visuel des objets stockés
- Nécessite Attribute Rendering Library

??? info "Mods liés"
    - :material-alert: [Attribute Rendering Lib](bibliotheques.md) : REQUIS pour fonctionner
    - :material-arrow-right: [Combat Overhaul](../gameplay/combat.md#combat-overhaul) : Rangement des équipements de combat

---

## Conseils

!!! tip "Carry On + Déménagement"
    Utilisez Carry On pour déplacer vos coffres pleins lors d'un déménagement. Énorme gain de temps !

!!! tip "Organisation optimale"
    Combinez Food Shelves pour la nourriture et Sortable Storage pour les ressources. Votre base sera toujours rangée !
